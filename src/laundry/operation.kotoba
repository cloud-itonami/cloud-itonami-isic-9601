(ns laundry.operation
  "OperationActor -- one garment-care operation = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor
  (LaundryOps-LLM) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the Garment Care Governor
  ...
  One graph run = one garment-care operation (intake -> advise ->
  ...
  decision to a human operator (laundry/dry-cleaning staff). The
  approver resumes with `{:approval {:status :approved}}` (or
  :rejected). `:actuation/apply-cleaning-process`/`:actuation/return-
  garment` ALWAYS reach this node when the governor is clean -- see
  `laundry.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [laundry.laundryadvisor :as laundryadvisor]
...
(defn build
  ...
  `laundry.store/Store`).
  opts:
    :advisor      -- a `laundry.laundryadvisor/Advisor` (default: mock-advisor)
    :checkpointer -- langgraph checkpointer (default: in-mem)"
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (laundryadvisor/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; LaundryOps-LLM inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (laundryadvisor/-advise advisor store request)]
            {:proposal p :audit [(laundryadvisor/trace request p)]})))

      ;; Garment Care Governor -- independent censor (separate system than the LLM).
      (g/add-node :govern
